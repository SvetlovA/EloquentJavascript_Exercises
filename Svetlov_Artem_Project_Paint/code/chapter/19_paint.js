...
tools.Rectangle = function (event, cx) {
    ...
    var currentPos = relativePos(event, cx.canvas);
    trackDrag(function (event) {
        cx.clearRect(startPos.x, startPos.y, currentPos.x - startPos.x + 2, currentPos.y - startPos.y + 2);
        cx.clearRect(startPos.x, startPos.y, currentPos.x - startPos.x - 2, currentPos.y - startPos.y - 2);
        cx.clearRect(startPos.x, startPos.y, currentPos.x - startPos.x + 2, currentPos.y - startPos.y - 2);
        cx.clearRect(startPos.x, startPos.y, currentPos.x - startPos.x - 2, currentPos.y - startPos.y + 2);
        currentPos = relativePos(event, cx.canvas);
        cx.strokeRect(startPos.x, startPos.y, currentPos.x - startPos.x, currentPos.y - startPos.y);
    }, function () {
        cx.strokeRect(startPos.x, startPos.y, currentPos.x - startPos.x, currentPos.y - startPos.y);
    });
};
...
tools.GetColor = function (event, cx) {
    var pos = relativePos(event, cx.canvas);
    var color = pixelAt(cx, pos.x, pos.y);
    cx.fillStyle = "rgba(" + color[0] + ", " + color[1] + ", " + color[2] + ", " + color[3] + ")";
    cx.strokeStyle = "rgba(" + color[0] + ", " + color[1] + ", " + color[2] + ", " + color[3] + ")";
};
